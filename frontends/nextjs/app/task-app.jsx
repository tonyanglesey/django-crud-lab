...
export default function TaskApp({ initialData }) {
  const [tasks, setTasks] = useState(initialData.tasks);
  const [counts, setCounts] = useState(initialData.counts);
  const [choices] = useState(initialData.choices);
  ...
  async function loadTasks(nextFilters = filters) {
    ...
    const payload = await api(`/api/tasks${params.toString() ? `?${params}` : ''}`);
    setTasks(payload.tasks);
    setCounts(payload.counts);
  }

  async function setFilter(nextFilters) {
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <header className="mb-8 grid gap-6 lg:grid-cols-[1fr_320px] lg:items-end">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-lg leading-8 text-stone">
            Next renders initial data on the server, then uses route handlers to proxy client mutations to Django.
          </p>
        </div>
      ...
      <section className="mb-6 grid gap-3 border-y border-mist py-4 lg:grid-cols-[1fr_auto] lg:items-center">
        <input
          ...
          value={filters.q}
          onChange={(event) => setFilter({ ...filters, q: event.target.value })}
          placeholder="Search tasks"
        />
        <div className="flex flex-wrap gap-2 lg:justify-end">
          <button className={`rounded-lg px-4 py-2 text-sm font-bold transition ${!filters.status ? 'bg-ink text-white' : 'bg-white text-stone hover:bg-mist'}`} onClick={() => setFilter({ ...filters, status: '' })} type="button">All</button>
          {choices.statuses.map((status) => (
            <button className={`rounded-lg px-4 py-2 text-sm font-bold transition ${filters.status === status.value ? 'bg-ink text-white' : 'bg-white text-stone hover:bg-mist'}`} key={status.value} onClick={() => setFilter({ ...filters, status: status.value })} type="button">
              {status.label}
            </button>
          ))}
        </div>
...
function TaskForm({ choices, draft, editingId, errors, onCancel, onChange, onSubmit }) {
  return (
    <form className="rounded-lg border border-mist bg-white/90 p-5 shadow-soft" onSubmit={onSubmit}>
      <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.18em] text-forest-600">
        {editingId ? 'Edit through Next.js' : 'Create through Next.js'}
      </p>
      <h2 className="font-display text-3xl font-bold">{editingId ? 'Edit task' : 'New task'}</h2>
      {errors.__all__?.map((error) => <p className="mt-4 rounded-lg bg-clay-100 p-3 text-sm font-semibold text-clay-800" key={error}>{error}</p>)}
      <TaskField label="Title" errors={errors.title}>
        <input className="field-control" required value={draft.title} onChange={(event) => onChange({ ...draft, title: event.target.value })} />
      </TaskField>
      <TaskField label="Description" errors={errors.description}>
        <textarea className="field-control" rows="4" value={draft.description} onChange={(event) => onChange({ ...draft, description: event.target.value })} />
      </TaskField>
      <TaskField label="Status" errors={errors.status}>
        <select className="field-control" value={draft.status} onChange={(event) => onChange({ ...draft, status: event.target.value })}>
      ...
      <TaskField label="Priority" errors={errors.priority}>
        <select className="field-control" value={draft.priority} onChange={(event) => onChange({ ...draft, priority: event.target.value })}>
      ...
      <TaskField label="Due date" errors={errors.due_date}>
        <input className="field-control" type="date" value={draft.due_date} onChange={(event) => onChange({ ...draft, due_date: event.target.value })} />
      </TaskField>
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        {editingId && <button className="rounded-lg border border-mist bg-white px-5 py-3 text-sm font-bold text-ink transition hover:bg-paper" onClick={onCancel} type="button">Cancel</button>}
      </div>
    </form>
  );
}
...
function TaskField({ label, errors = [], children }) {
  return (
    <label className="mt-5 block text-sm font-extrabold text-ink">
      {label}
      {children}
      {errors.map((error) => <p className="mt-2 text-sm font-semibold text-clay-800" key={error}>{error}</p>)}
    </label>
  );
}
